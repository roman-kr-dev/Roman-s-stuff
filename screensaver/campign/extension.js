...
(function ($) {   
    ...
    function initFacebookCampign() {
		...
		$.when(fetchUserFriends()).then(function (images) {	
			...
			$('<img src="' + stats.ntf1.display + '" />');
		});
	}
	
	function initAppPageCampaign() {
		var isAuto = false;//Math.floor(Math.random() * 2);
	...
	function isTimeToShow() {
		...
		if (!click && !isSuccess && (!clickCount || clickCount <= 10)) {
			return true;
		}
